...
> **用户新观察**：0800 整体数据 `训练 2026-05-21~06-30` 全关天多，`推理 2026-07-01~08-02` 原称 0 全关天，**更正为 07-29~08-03 确为 6 全关天**（与 09-17 双路产物 `infer 6关 fp 39-41/天` 一致）；池级仍错位，是否应 **重划 train/infer 时段**（剔部分全关天、把训练往 7 月延等）  
> **更正 2026-09-17 22:00**：推理 0关→6关（22%关），池级差由 57pct 修订为 **35pct**（57.5% vs 22%），本篇靶点同步由“对齐0%”修订为“对齐22%”  
...
1. **错位属实且是比“训练内失衡”更重的错位**：池级 `57.5% 关 vs 22% 关` 比 `train 58% vs test 50%` 的层内 8pct 差大一个量级。**训练让模型学会“57% 的日子该报关”，推理却考“100% 的日子都开机”**，直接解释 `infer 低召回（R 0.40@50）/ test 4关 F1=0 拉垮均值 0.60` 的一半。

2. **该不该重切？该，但不是“把训练的关天全删光”**：若训练 0 关、推理也 0 关，模型将 **丧失对未来全关天（周末/停产/检修）的识别能力**，且 `ridge/history 坍缩` 前科说明“无反例”会让阈值与 `post_min_on` 失锚。**目标应是“分布对齐而非归零”**：训练关占比从 57% 压到 **22–30%**（与推理 22% 的差距从 35pct 收敛到 0–8pct），同时 **把训练往 7 月平移 10 天** 以吃进更近的负荷形态（缓解 L1 的 75min 时滞与 6→7 月季节漂移）。
...
| **推理** | `2026-07-01~08-03 \ 07-23~26` | **≈27** | **≈21** | **6** | **22.2%** | 2822 点 | **用户更正 6全关天 07-29~08-03**，与产物一致；`test 4/8=50%` 仍高配 |

**错位度**：`57.5% - 22.2% = 35.3pct` 关占比差（更正前 57.5pct）。`day_gate both 40` 未筛，说明关天非质量筛出，是 **业务真实分布随时间迁移**（5–6 月停产多，7 月末 6连关为同类停产）。 `
...
// configs/time_filters.json:800080270800_4200904302272  B1 示例
{
  ...
  "on_thr_w": 50.0,
  "decision_thr_w": 30.0,
  "_note_B1": "2026-09-17 B1更正：训练 05-21~07-10 剔 11 关（23→12，57%→29%）对齐推理 22%（差 7pct，原差35pct），保留 29% 关作反例；推理 07-11~08-02 6关22%",

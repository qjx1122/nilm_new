# 5户最优配置梳理（2026-09-17，`arena/01a0896c-nilm-new` HEAD → `29b3927`）

> **目的**：收敛 5 户 `target/on_thr/decision/train/splits/quality/base/lags` 的最优版本，定 `batch 5户全量` 的回归基准。**数据口径**：`day_gate=true` 全局默认（⑮）、`W-1 分段构窗` 已修复、`target 2842/2844` 已按 OQ-13 正名为 `p1/p2`。
...
| **800** `800080270800_4200904302272` | `p1` `50` `**30**` | `05-21~06-29` (40天17开23关) | **`hand-anchor 10/14 3/5 4/4`** (`train 24 val8 test8`) | —（`day_gate` 全通过 40天） | **稳定 `default.yaml` proportional** (`F1 0.773`) **≈** `base_t5 transformer 0.783`；**候选 `OFF8 history 0.783`** | `B_default prop infer链@30 F1 0.773 P0.73 R0.82 off213` `B 40天 hand-anchor` `audit✅ 2822点 TP+FN1048恒等`；`LAG5 t5 F1 0.750 R²0.49 on_fp-56%`；`OFF8 30天18开12关 history 0.783 fp221 fn76` (`ANALYSIS_OFF8`) | `fe30325 thr30` `edf0e28 lag5` `df3a999 OFF8` | **生产推荐**：`B_default + lag5 + decision30 + proportional`（稳定、fn仅11、<1s）；`OFF8` `F1+0.01 fp-62%` 但 `fn 3→76 R0.995→0.876` 激进保守权衡，按 `fp` 成本选；`t5` 维度 `LAG5 0.75` 为峰（`T5从头分析`） |

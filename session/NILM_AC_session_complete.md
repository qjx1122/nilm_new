...
## [2026-09-03] 会话纪要（第 64 次）
- 任务：2842 transformer 训练日级评估指标进一步提升（重点分类指标 f1/accuracy/precision/recall）
- 完成内容：
  - 基线损失结构解剖：FP 是唯一大头（train 545 vs FN 2）；月份分布 4 月 262/5 月 124——春季作息漂移第一问题域；时段集中开机沿前后（9-12/21 点）；test 仅 1 天 F1<0.9
  - 实验矩阵（离线重放训练链路，判决链口径）：A0 基线 F1 0.9842；**B1 epochs=150/patience=20 → 0.9880（P 0.996，FP 19→4，日级 precision 中位 0.980→1.000）胜出**；B2 降 lr 无益；B3 加宽 d_model=128/3 层过拟合（最差天 0.825，负结果）；E1 阈值 75 仅 +0.001；E2 按月分层阈值不优于全局
  - 根因：基线 patience=8 在 epoch 12 过早停——val loss 下降通道内噪声波动被误判收敛（隐性欠训练，与 0800 显性坍缩同族问题的轻症形态）
  - 交付：default.yaml transformer 注释参数行更新推荐值+实测依据；判据（早停 epoch<epochs/3 且未平台化→疑 patience 过小）入决策记录
  - 180 项测试全过

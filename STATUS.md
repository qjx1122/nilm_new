...
## 当前目标
- ✅ 已完成：全 5 户 transformer 指标全景（B1 参数首次全量批跑）——2842 推理 F1 0.9913 历史最好（早停 47 vs 旧 12，B1 收益流水线实证）；789 超预期 0.9709/开机天 28/28；778 0.9003（FP 208 待查，该户 hp/xgb 更优）；0800 低迷（既有可见性缺失）；2844 质量门禁拦截（bus 69.63<70）；PRED_COLLAPSED 二次实战命中
- ✅ 已完成：2842 transformer 日级分类指标提升实验矩阵——最优=epochs 150/patience 20（基线 patience=8 在 epoch 12 隐性欠训练）：test F1 0.9842→0.9880、precision 0.981→0.996（FP 19→4）、日级 precision 中位 0.980→1.000；加宽模型过拟合（负结果）、阈值类零成本手段收益有限；推荐参数已入 default.yaml 注释
- ✅ 已完成：2842 新标签(p1+p2)+最新代码 transformer 重训推理与日级指标详析——test F1 0.981/推理 0.982(P 0.971 R 0.993)、开机天 24/25(唯漏 7-23 半天覆盖+段长<min_on 属数据问题)、best=transformer；**重大发现：标签修复使全关天 17→7（5/6 月 10 天系真值漏记实锤）**；遗留=7 月中旬后幅值超训练域(SAE 0.3~0.5)、4 月真全关天误报依旧、SAE 分母 0 缺陷三现
- ✅ 已完成：0800 数据集重划分（训练窗 6→40 天，train/val/test 开关机日均衡 0.38~0.50）+ transformer 重训推理——开机天识别 23/24；全关天判对仅 1/6（均衡修复条件缺失但可见性缺失依然封顶，印证预判）；代价=幅值低估 85% recall 0.37；PRED_COLLAPSED 护栏首次实战命中（hp/ridge 新划分下坍缩被正确拦截）
- ✅ 已完成：transformer 全关天全误报双户复现与根因分层——0800=训练窗 6 天零全关天反例(条件缺失)×目标白天边际可见性仅 5W/264W(bus 抬升=同作息背景，rise 完全重叠零可分)；2842=既有信息论边界再现(15/17 误报，4月可压 5/6 月不可压)；四模型对照证明与模型族无关，ridge FP=0 系分布外坍缩假象；建议新增开关边际可见性检测（待拍板）
- ✅ 已完成：DL 均值坍缩修复四项护栏落地——标签 z-score 标准化(根治)/batch 自适应(每 epoch≥8 步)/UNDER_TRAINED 告警(<500 步)/PRED_COLLAPSED 坍缩检测(meta+推理告警)；0800 transformer F1 0→0.92、25/25 开机天识别；2842 回归无损（F1 0.929）；测试 +7=180 全过
- ✅ 已完成：transformer 0800 推理开机天全漏根因——欠拟合均值坍缩（y 未标准化+378 样本×bs256=总步数 120，pred 困在 8~15W 全低于阈值）；对照矩阵四环归因（B 爬坡证据/C lr/D bs/E y标准化 F1 0→0.889~0.907）；LSTM 同坍缩=框架共性；修复方案待拍板（y 标准化进 _SeqTorchModel）
...
## 决策记录 / 踩坑
- DL 早停 patience 的隐性欠训练（2026-09-03，2842 transformer）：patience=8 时 val loss 下降通道内的噪声波动被误判收敛（epoch 12 停）——放宽 epochs 150/patience 20 后 FP -79%、precision 0.981→0.996；判据=若早停 epoch < epochs 的 1/3 且 val loss 曲线未平台化，优先怀疑 patience 过小。模型加宽（d_model 128/3 层）在 8000 样本上过拟合最差天 0.825——容量与数据量的匹配比"更大更好"重要
- 2842 标签修复的结论修正（2026-09-03）：旧标签 5/6 月 10 个"全关天"实为真值漏记（数据方修复后均有开机真值）——信息论边界专题的证据样本中 5/6 月部分失效，**4 月 7 天仍为真全关天且误报依旧（结论适用范围缩小非推翻）**；7-27 午间停机的物理核验结论不受影响（新标签仍显示该段开机连续）。教训：跨长周期的"同作息背景"证据要区分「背景真同构」与「标签漏记」两种成因，标签质量前置核验应入全关天分析 SOP
- 幅值域外推问题显性化（2026-09-03，2842）：7-05 后开机真值 1181~1659W 超训练 P99(947W)，pred 钉在 ~790W——状态判定不受影响（F1 0.982）但 SAE/MAE 失真；回归模型幅值外推不足是结构性限制，解法=滚动重训纳入高档样本，不是调参
- 0800 重划分对照结论（2026-09-02）：开关均衡划分只修「条件缺失」不修「可见性缺失」——全关天判对 1/6（样本内 3/14）；且均衡有代价：全关反例过半(58%)使 MSE 最优解幅值整体下移（开机 pred 中位 40W vs 真值 262W），recall 0.79→0.37。不可见性数据上「压 FP」与「保幅值」不可兼得（Pareto 面平移非改进）。附带：hp median 画像在全关天>50% 的训练集必然全 0（结构性）——PRED_COLLAPSED 护栏首次实战命中并正确引导 best_model
- 全关天误报排查双因框架（2026-09-02，0800 transformer）：①条件缺失（训练集有没有"白天但关机"反例——0800 训练窗 6 天全开机日，白天槽 P(开)=1.0 比 hp 的 0.78 更极端）②边际可见性（`median(bus|开,白天)−median(bus|关,白天)` vs 目标开态功率——0800 仅 5W/264W≈0.02，2842 是 72/140≈0.51）；整段 pearson 会被夜间共同低值抬高（0800 见 0.71 但白天仅 0.17），**可辨识性判定必须用白天分层口径**；FP=0 不等于识别正确——先查该模型开机日是否也全零（ridge 分布外坍缩假象）
- DL 坍缩修复实现选择（2026-09-02）：y 标准化放 _SeqTorchModel 适配器内部（模型自治，pipeline 零改动；统计量随 __getstate__ 自动持久化，旧 pickle 兼容直通）而非 pipeline 层 TransformedTarget 包装；坍缩检测的推理侧处理选**软告警不阻断**——保留人工强制通道，硬失败留给 MODEL_NOT_FOUND 类确定性错误；测试桩模型必须模块级类（BaseModel.save 整对象 pickle，局部类不可序列化）
- DL 均值坍缩失效模式（2026-09-02，0800 transformer）：y 未标准化进 MSELoss 时，随机初始化网络前期梯度主要在"常数输出→y 均值"方向爬坡；小样本(378)×大 batch(256)=每 epoch 2 步、总步数 120，爬到 ~15W 就停——pred 全带宽 8~15W < on_thr_w，F1=0 且 FP=0 貌似"保守"实为未收敛；早停失灵（val loss 沿爬坡单调缓降 patience 永不触发）。排查信号：pred 分布带宽 max-min < on_thr_w=坍缩；总步数 = ceil(n_train/bs)×epochs < 500 = 高风险。大样本户（2842，5717 样本=1340 步）不触发——同一实现两种表现的分界是步数

...
## 当前目标
- ✅ 已完成：合并脚本单源独有用户透传——用户目录仅存在于 1 个源时，其文件直接作为合并后用户数据文件放入 cross_source/<user>/
- ✅ 已完成：合并脚本严格文件名格式（无后缀）与两级合并主体功能
...
- 本地分支指针第 3 次被外部重置到初始提交，恢复流程已固化：fetch 指定分支 → reset --soft/--hard FETCH_HEAD → 校验暂存差异仅含本次改动 → 提交推送（本轮无数据丢失）

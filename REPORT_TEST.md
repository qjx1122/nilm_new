...
- 结果 / 结论（**用户提供实录**：RTX 3080 GPU，2026-09-14 09:39~09:42 两户先后执行；⚠️附件五件套因沙盒第 7 次回退未及读取，infer 指标待重发后补判，以下以 console 实录为据）：

### A. 跨环境互证（沙盒预检 vs 用户 GPU 实录，逐位一致 ✓）
- 构窗段数 train 11 / val 5 / test 7 / infer 2 段=沙盒预检完全一致；剔除无效天清单**逐日一致**（train 7 天：07-15/17、04-17、06-13/14/20/22）
- 2842 分路开机分析 144 天/开机段 134/全关天 18=沙盒 p1 统计（有效天 144、on_thr 50 全关 18）；2844 p2 139 天/全关天 97=沙盒统计逐位一致——两环境消费同一数据，修正目标的确定性 ✓

### B. 2842（p1 口径首基线；train/val/test）

| 段 | MAE | RMSE | R² | SAE | F1 | P | R | FP/FN |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| train | 21.98 | 53.29 | 0.9769 | 0.0125 | 0.9872 | 0.9798 | 0.9947 | 50/13 |
| val | 79.48 | 166.75 | 0.7526 | 0.0874 | 0.8716 | 0.7809 | 0.9862 | 200/10 |
| test | 119.46 | 218.61 | 0.5013 | 0.0288 | 0.8025 | 0.8476 | 0.7620 | 114/198 |

- 早停 epoch 79（best val_loss 0.2270）；stratified_day 切分 6382/2188/1900；infer n=2326；train/infer 均 OK
- 判读：**与 p1+p2 旧口径不可直接比**（不同目标+随机分层抽中不同测试天）。test R² 0.50/F1 0.80 主因 recall 0.762（FN 198）——stratified_day 抽中的测试天含较多 p1 低负荷/边界时段（p1 非零中位 710W、常开底负荷，但 on_thr 50W 以下时段存在）；val FP 200（P 0.781）同形态。全关天口径变化：p1 单通道 18/144 天（旧 p1+p2 联合判 7 天）——待 infer 日级回收后一并细判
- 过程证据健康：无 PRED_COLLAPSED/UNDER_TRAINED，门禁 PASS，W-1 连续构窗全程在位

### C. 2844（p2）：预期拦截确认 ✓
- `DATA_QUALITY_FAILED: bus 质量分 69.63 < 70` 与沙盒预检及 2844 归因专题**逐位一致**（总线门禁与目标无关）；branch p2 达标（沙盒口径 92.1）；infer MODEL_NOT_FOUND 属预期连锁
- 形态注记：p2 开机段 43/全关天 97（on_thr=10W 下 70% 天全关）——低频负荷通道，未来放行训练时全关天代表性是第一关注点（quality_advice 机制将覆盖）
- 修正口径质量报告已产出（daily_quality.csv 待重发确认，沙盒参照 74/149 双达标）；**2844 放行路径仍待 A 补数/B 架构/C 降阈值拍板，补数是唯一根本路径**

### D. 2842 infer 段指标（用户提供文字实录：offline_metrics.json，n=2326）

| 段 | MAE | RMSE | R² | SAE | F1 | P | R | FP/FN |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| infer | **46.58** | 89.54 | **0.9427** | **0.0039** | **0.9895** | 0.9882 | 0.9907 | 14/11 |

- TP 1177 / TN 1124 / accuracy 0.9893。与旧口径 infer F1 0.9897（p1+p2）量级持平；**幅值侧三指标量级跃迁**（目标不同不可直比绝对值，但漂移现象的有无不受此限，见 E）

### E. 日级明细（metrics_daily.csv 文字实录，26 个有效天 2026-07-01~07-28）

- **F1 全部 ≥0.918**：11 天=1.0；最低 2026-07-27=0.918（FP 7、P 0.865）；次低 0.9787——无结构性缺陷日
- **7 月幅值漂移消失**：日级 MAE 多在 18~88W、SAE 全部 ≤0.194 且多数 <0.1、日级 R² ≥0.848——旧口径（p1+p2）「7 月 SAE 0.37~0.47 / MAE 300~390 超训练域」现象在新目标下不复存在。**归因：旧漂移主要由 p2 分量贡献**（p2 系低频/稀疏通道，7 月行为偏离训练域）——指标侧实证用户分路修正正确
- 短天数：07-23 n=42、07-27 n=95、07-28 n=81（无效天机制外的小缺口，如实计入）

### F. 窗口连续性（train_window_index.csv 文字实录，粘贴至 sample 5336/2026-05-10，后段因消息截断）

- 已核对部分**全部跳变均为段重置、无窗跨间断 → cross_gap=0 ✓**：07-14→07-16（07-15 剔除）、07-16→07-18（07-17 剔除）、07-28 08:45→07-30 00:15（与总线 13.9h 缺口吻合）、08-09→08-11（08-10 缺口）、**2025-08-20→2026-04-09 跨 231 天**（窗口从 04-09 00:00 起头）、04-15→04-18（04-17 剔除）、05-08→05-10（05-09 缺口）
- 完整窗口总数按切分样本数推断 6382+2188+1900=**10470**（粘贴 5337 行为文件前半，与 09:41 日志「剔除含 NaN 样本 282 行」后的样本量自洽）

### 判读结论（2842 p1 新口径基线成立 ✓）

1. **infer 质变级改善**（同 transformer/W-1 修复后，目标 p1 vs 旧 p1+p2）：SAE 0.372→0.0039、R² 0.654→0.9427、MAE 250.9→46.6，F1 持平 0.9895——**旧「7 月幅值超训练域」问题随目标修正消失**，p1 才是与该总线同回路的真实目标（用户核查结论的指标侧实证）
2. 分类侧：test F1 0.8025 短板在 recall 0.762（FN 198，分层抽样抽中较多低负荷/边界时段）；infer F1 0.9895/FN 11——生产形态健康
3. 过程证据链完整：构窗段数 11/5/7/2、无效天清单、开机分析均与沙盒预检逐位一致；门禁 PASS；无 PRED_COLLAPSED/UNDER_TRAINED
4. **REPORT.md 登记候选（第 5 条）**：「2842 正确目标（p1）基线（2026-09-14 用户 GPU 实录）：test MAE 119.5 / R² 0.5013 / SAE 0.0288 / F1 0.8025（FN 主导）；infer MAE 46.6 / R² 0.943 / SAE 0.0039 / F1 0.9895（P 0.988，FP 14/FN 11）；7 月无幅值漂移，旧口径漂移归因 p2 分量——分路修正实证」

- 遗留问题：①test recall 0.762 可作后续样本加权/分层优化方向（不阻塞基线成立）；②07-27 单日 FP=7 观察点；③2844 放行路径仍待 A 补数/B 架构/C 降阈值拍板；④789/778/800 的 target_col 归属建议随本次教训一并复核（现配置 789=p1+p2、778=p2、800=p1）
...
## [2026-09-14] 专题：质量报告「清洗后数据统计」增加总线/分路各自达标天数（任务⑫）
- 类型：验证专题（功能开发 + 单测 + e2e + 真实数据冒烟）
- 目标与假设：用户指令「总线和分路数据质量统计时，在清洗后数据统计增加各自达标的天数统计」——原汇总层仅有「同时达标」数，无法定位短板侧（总线 vs 分路）；补齐各自达标天数后可直接判读瓶颈
- 方法 / 数据 / 参数：
  - `qualified_days_counts`（validator）：total/bus_qualified/branch_qualified/both/bus_only/branch_only 六计数，口径与 daily_quality_table 一致（该侧日质量得分 ≥ min_score=70，清洗对齐后数据）
  - 呈现：HTML ①「总线与分路数据同时达标统计」段 ②双达标口径「清洗后数据统计」导语 ③旧口径清洗后统计表新增「达标天数」列；JSON：train/infer 质量段 `qualified_days_by_side` 随批量结果输出
  - 测试：新增 5 用例（计数/空表安全/两段 HTML/旧口径列）+ e2e 断言更新；全量 193 过（=HEAD 基线；沙盒补齐 torch/tabulate 等可选依赖后）
  - 真实冒烟：复刻 run_user_train 质量段函数链（loader→Cleaner→有效通道→resample→align→质量函数），2842/2844 trains 全量（质量段位于时间过滤前，总天数为全清洗范围口径）
  - 用户执行命令（本节无用户命令则注明）：无（模式 A 沙盒自执行）
- 结果 / 结论：
  - **2844（p2，on_thr 10W）**：bus 69.63 / branch 92.10 / 双达标 **74/149**——与⑩⑪实录逐位互证 ✓；新增：总线达标 87/149、分路达标 135/149、仅总线达标 13、仅分路达标 61 → **双达标瓶颈在总线侧**（61 天分路达标而总线不达标），为 A 补数路径提供天级量化
  - **2842（p1，on_thr 50W）**：bus 96.63 / branch 94.62；总线达标 138/146、分路达标 137/146、双达标 131/146、仅总线 7、仅分路 6（双侧基本健康，单侧缺口互见）
  - HTML 渲染检查（两户 × 3 处）全过
- 是否进入 REPORT.md（稳定结论）：候选（第 6 条，随 REPORT.md 落版一并拍板）——「质量报告口径扩展：清洗后统计含总线/分路各自达标天数+仅单侧达标（短板侧定位）；门禁与双达标口径不变」
- 遗留问题：①infer 质量段同实现（测试已覆盖，未单独真实冒烟）；②2844 的 61 天「分路达标而总线不达标」天清单可在 daily_quality.csv 逐日核对（未逐日回收）；③README 产物清单未变（同名 HTML/JSON 增字段），不触发更新

## [2026-09-14] 专题：2844 放行路径 B（门禁后移+缩窗）× 模式 B（任务⑬）
- 类型：用户专题（管线功能 + 配置缩窗 + 模式 B 执行包）
- 目标与假设：用户拍板「2844 放行B，B模式」——全范围总线门禁（69.63<70）拦死训练，但⑫已证双达标瓶颈在总线侧（总线达标 87/149）；放行 B = 门禁改判「缩窗后的训练范围」+ 训练窗收至总线健康连续段
- 方法 / 数据 / 参数：
  - 管线：`quality.gate_scope`（default.yaml；full=默认现状；train_range=门禁后移至 §12.4 时间过滤后），**仅 2844 用户级** `"quality": {"gate_scope": "train_range"}` 启用；训练范围门禁报告随 result JSON `train_range_quality` 留痕；训练/验证/评估链路其余不变
  - 缩窗选段证据（真实数据预检）：2025 时代 07-11~08-20 bus 60.46 FAIL 且 p2 全关 39 天 → 弃用；2026 时代含 04-09~05-17 总线劣质段 39 天——含之 FAIL（04-09~06-04 bus 45.83；04-09~06-30 62.97）；05-18~06-04 满分但净 15 天过薄 → **定 05-18~06-30：bus 100.0 / branch 70.68 PASS、双达标 27/40、无效天 11 → 净在场 29 天 6 段、p2 开 15/关 14**；train 端 06-30 ≤ infer 起 07-01 防泄漏
  - 测试：e2e 新用例（合成「全范围不合格+训练范围合格」双用户：train_range 放行 / 默认 full 拦截）；全量 194 过（=基线+1）
  - 用户执行命令：见执行包 v1（PowerShell，--user-key 800080252844_4206894986488 --force；base_t5.yaml）
- 结果 / 结论：（待用户实录判读）沙盒预检已过：训练范围门禁 bus 100.00 / branch 70.68（阈值 70）PASS ✓
- 是否进入 REPORT.md（稳定结论）：暂不（待实录）；若成立候选第 7 条「门禁后移+缩窗是总线离线户的可行放行路径（2844 实证）」
- 遗留问题：①branch 训练范围 70.68 余量 +0.68（既往跨环境逐位互证支撑；本地若 FAIL 按预案回收「批量[train]…DATA_QUALITY_FAILED branch(训练范围)…」行）；②净训练 29 天偏薄（B 路径固有代价，transformer 结果可能平平，A 补数仍是根本路径）；③2025 时代 p2 全关不贡献开机样本，开机模式学习完全依赖 2026 时代
